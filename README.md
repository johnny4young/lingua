...
- **Run Capsules**: portable, redacted JSON captures of a run (source, output, input, environment) with confirm-first import, a Pro capsule browser with compare, and one-click export to a self-contained syntax-highlighted HTML document that opens anywhere with zero network requests.
...
- **HTTP workspace**: compose and send requests with reusable secret-aware `{{variable}}` environments, response-variable capture for request chaining, persisted response assertions, cURL/Postman import, and Bruno request or collection-folder import across classic `.bru` and OpenCollection YAML; see the [import guide](./docs/IMPORTING.md). Copy-as-code supports cURL, `fetch`, `axios`, and Python `requests`.
