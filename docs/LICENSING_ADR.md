...
- `RL-061` scope expands to include Polar product setup for the
  public paid tiers (Monthly + Pro) and webhook wiring (`order.paid`,
  `order.refunded`, `subscription.created`, `subscription.updated`,
  `subscription.canceled`). Internal product slugs may keep legacy
  names until an explicit backend migration renames them; public copy
  must use Free, Monthly, Pro, and Education.
...
- The canonical Lingua SKU identifier in every Polar webhook payload
  ...
  setting `metadata.product_id` to the approved backend product slug
  for Monthly or Pro when creating each product in the Polar dashboard.
  A webhook missing or carrying an unknown
...
- **Free** — no purchase, hard Free ceilings in the renderer.
- **Monthly** — subscription, hard 3-device limit.
- **Pro** — one-time purchase, hard 3-device limit.
- **Education** — free 1-year renewable education license, hard
  3-device limit.

Implementation mapping:

- **`lingua_monthly`** backs the public Monthly tier.
- The public Pro one-time tier may still use the legacy
  **`lingua_lifetime`** backend slug until a dedicated migration
  renames it. Public copy must call this tier **Pro**, not Lifetime.
- **`lingua_education`** backs the public Education tier and never
  enters the Polar product catalog.
- **`tier: 'trial'`** remains an internal onboarding/support token
  shape, not a public pricing tier.

- **`tier: 'trial'`** — 14 days, 1 device, full paid entitlements,
  one-shot. Minted by `/trials/start`. Same anti-abuse as below
  (UNIQUE email + UNIQUE device).
- **`tier: 'education'`** — 1 year, full paid entitlements,
...
The paid-tier device cap (3) applies to the education tier as well —
students reinstalling on a new laptop go through the same
...
- `licenses` table carries `device_limit INTEGER NOT NULL DEFAULT 3`
  so the gate runs in SQL on activation. Education shares the hard-3
  default with Monthly + Pro.
- Education + trial bypass the webhook entirely — they are
  server-minted by their dedicated endpoints.
...
- Settings → License surface displays `(N of {device_limit})`; for the
  public launch tiers this is normally `3`.
