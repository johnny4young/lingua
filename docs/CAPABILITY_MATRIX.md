...
| Dependency management | Out | **Detection only (web)** | Research | **Shipping (detection + JS/TS install)** | **Hybrid** | Hybrid (detection web + desktop, install desktop-first) | RL-025 Slice A (2026-05-22) + Slice B (2026-05-24). Slice A detects external imports without IPC via `src/shared/dependencies/{javascriptDetector,pythonDetector,registry}.ts`; main-side `resolveJsDependencyBatch` walks `node_modules` for JS/TS; web build short-circuits to `'needs-desktop'`. **Slice B** shipped JS/TS desktop install via `installJsDependencyBatch` in `src/main/dependencies.ts` — `child_process.spawn('npm', ['install', …names, '--no-audit', '--no-fund', '--no-progress', '--save'], { shell: false, cwd, env })`; refuses without an upstream `package.json` (anti-feature: never silently turn a scratchpad into a project), pre-flights against the Slice A resolver, supports SIGTERM/SIGKILL cancel and a 5-min hard timeout, streams stdout/stderr via `webContents.send('dependencies:js:install:log', …)`, and emits three closed-enum telemetry events (`install_started`, `install_completed`, `install_failed_reason`) mirrored on update-server with parity test. Slice C will add Python web install via `micropip`. Promotion rules for Ruby / Go / Rust / Python-venv / WebContainer install adapters live in `docs/DEPENDENCY_MANAGER_ADR.md` |
