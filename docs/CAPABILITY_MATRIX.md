...
| Notebook workspace     | **Shipping** | **Shipping**        | Out          | **Shipping**   | **Hybrid** | Browser interpreter + hybrid (Python) + Browser WASM (SQL) | TS/JS cells run in the shared worker (`notebookSession.ts` → `runnerManager`); Python cells ride the same Pyodide path (web + desktop) as the Python runner, hence hybrid. implementation gives Python cells a **per-notebook kernel scope**: the worker runs each cell against a namespace dict keyed by the notebook tabId (`notebookSession` passes `scopeId`), so cells in one notebook share Python state while notebook A / notebook B / the editor scratchpad stay isolated; Restart kernel + tab close send `reset-scope`. See `PYTHON_NOTEBOOK_KERNEL_ADR.md`. implementation adds SQL cells that run on the shared DuckDB-WASM engine (`executeQuery`) — the result set is emitted as a JSON-array output the rich-output layer renders as a table, and because the engine is a renderer-wide singleton, SQL cells share table state with each other and the SQL workspace. `'sql'` is a notebook-only cell language (NOT a global editor `LanguagePackId`). Persistence is localStorage today (`.linguanb` disk autosave is the pending slice). Cell list is doubly virtualized; only the focused cell mounts Monaco |
| HTTP workspace         | **Shipping** | **Shipping**        | Out          | **Shipping**   | —      | Browser interpreter (renderer `fetch`) | `src/renderer/runtime/httpClient.ts` uses the renderer's `fetch` on BOTH platforms today, so desktop shares the web CORS + safelisted-response-header constraints. internal implementation lands the desktop-native bypass engine (`src/main/httpProxy.ts`): an SSRF-guarded proxy that mirrors the `HttpResponseV1` envelope byte-for-byte while resolving every hop's host and rejecting loopback / link-local / RFC 1918 / CGNAT / ULA / multicast targets (opt-in `allowPrivateHosts` toggle), http/https-only, manual redirect re-guarding, and a `MAX_REDIRECTS` cap. The renderer transport seam + desktop Settings toggle that flip the HTTP workspace onto this engine (promoting desktop to its own column) are the tracked next slice; until then the engine is exercised by `tests/main/httpProxy.test.ts` only. Secret redaction is share/telemetry-time via `httpEnvironment.ts` |
