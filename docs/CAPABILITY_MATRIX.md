...
| HTTP workspace         | **Shipping** | **Shipping**        | Out          | **Shipping**   | —      | Browser interpreter (renderer `fetch`) | `src/renderer/runtime/httpClient.ts` uses the renderer's `fetch` on BOTH platforms — no main-process proxy today, so desktop shares the web CORS + safelisted-response-header constraints. A desktop-native `net.request` proxy to bypass CORS is the tracked next step (would promote desktop to its own column). Secret redaction is share/telemetry-time via `httpEnvironment.ts` |
