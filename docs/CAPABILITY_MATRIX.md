...
| Debugger (Python)      | Research     | Out                 | Out          | **Shipping (engine)** | — | Desktop native | implementation. `src/main/pythonDebugger.ts` drives a headless `python -u -m pdb <script>` subprocess (no shell): a `PythonDebugSession` sets breakpoints, continues, steps over/into/out, evaluates expressions, and parses pdb's REPL output into structured `{ file, line, func }` pause events + finished/exited signals. Commands are serialized to one prompt round-trip each. Validated against real CPython 3.11 pdb (`tests/main/pythonDebugger.test.ts`). The `debugger:python:*` IPC contract + preload bridge + the renderer debugger UI (breakpoint gutter, step toolbar, variables panel) are the following slice — the engine is designed to sit behind a thin marshaling layer like `git.ts`. Desktop-only: needs host `python3` (Pyodide/WASM has no pdb) |
