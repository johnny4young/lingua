...
| JS/TS runtime: Node    | Out          | Out                 | Research     | Planned (RL-019 Slice 2) | — | Desktop native | Spawns a Node child process with built-ins (`fs`, `path`, `http`). Slice 2 needs the child-process sandbox + timeout matrix landed under RL-078; the Slice 1 selector renders the option disabled with the same neutral "Coming soon" tooltip used by the UI |
