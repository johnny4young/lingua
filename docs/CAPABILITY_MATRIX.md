...
| Notebook workspace     | **Shipping** | **Shipping**        | Out          | **Shipping**   | **Hybrid** | Browser interpreter + hybrid (Python) | TS/JS cells run in the shared worker (`notebookSession.ts` → `runnerManager`); Python cells ride the same Pyodide path (web + desktop) as the Python runner, hence hybrid. T17 gives Python cells a **per-notebook kernel scope**: the worker runs each cell against a namespace dict keyed by the notebook tabId (`notebookSession` passes `scopeId`), so cells in one notebook share Python state while notebook A / notebook B / the editor scratchpad stay isolated; Restart kernel + tab close send `reset-scope`. See `PYTHON_NOTEBOOK_KERNEL_ADR.md`. Persistence is localStorage today (`.linguanb` disk autosave is the pending slice). Cell list is doubly virtualized; only the focused cell mounts Monaco |
