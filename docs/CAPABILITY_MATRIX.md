...
implementation — auto-derived from `LANGUAGE_SUPPORT_PROFILES` in
...
| Python | `available` | `available` | `planned` | `available` | `available` | `partial` | `available` | `available` | `desktop-only` |
| Go | `available` | `available` | `desktop-only` | `unsupported` | `available` | `desktop-only` | `desktop-only` | `unsupported` | `desktop-only` |
| Rust | `available` | `available` | `desktop-only` | `unsupported` | `available` | `desktop-only` | `desktop-only` | `unsupported` | `desktop-only` |
...
implementation — the Settings → Languages scorecard adds a
...
| Python | `available` | `available` | `planned` | `available` | `available` | `partial` | `available` | `available` | `unsupported` |
| Go | `available` | `available` | `unsupported` | `unsupported` | `available` | `unsupported` | `unsupported` | `unsupported` | `unsupported` |
| Rust | `available` | `available` | `unsupported` | `unsupported` | `available` | `unsupported` | `unsupported` | `unsupported` | `unsupported` |
...
| Python | `available` | `available` | `planned` | `available` | `available` | `partial` | `available` | `available` | `available` |
| Go | `available` | `available` | `available` | `unsupported` | `available` | `available` | `available` | `unsupported` | `available` |
| Rust | `available` | `available` | `available` | `unsupported` | `available` | `available` | `available` | `unsupported` | `available` |
...
| Ruby execution        | **Shipping**  | Out                 | Out          | **Shipping (hybrid)** | **Hybrid** | Browser WASM (web) + system ruby (desktop) | implementation shipped the WASM worker (`src/renderer/workers/ruby-worker.ts`). implementation added the desktop subprocess (`src/main/ruby-runner.ts` + `window.lingua.ruby.{detect,run,stop}`), with a renderer-side hybrid dispatcher (`RubyRunner` → `WasmRubyRunner` / `DesktopRubySubprocessRunner`) gated on Settings → Editor → "Ruby runtime" (`auto` / `system` / `wasm`). `.ruby-version` is honored via `RBENV_VERSION`. Bundled stdlib only on the WASM path; system gems on the subprocess path. Native gems via `bundler` belong to the internal lane |
...
| Project test runner    | Out          | Out                 | Out          | **Primary**    | —      | Desktop native | Detects project-local Vitest/Jest, Pytest configuration, `go.mod`, and `Cargo.toml` behind the approved root capability. Main re-detects before a fixed no-shell spawn, filters inherited environment variables, streams capped output, enforces one run per root, and owns stop/timeout. Web reports the honest desktop-only boundary |
| Integrated terminal    | Out          | Out                 | Research     | **Primary**    | —      | Desktop native | An explicit action opens the user's real system shell through `node-pty` and xterm. Main resolves the approved project root as the starting cwd, selects only an absolute host shell, filters inherited environment variables, binds sessions to the renderer owner, and tears them down on root revocation, renderer destruction, or app quit. The project root is not a sandbox: commands may navigate elsewhere with the user's OS permissions. Web omits the bridge and the surface |
| Local MCP server       | Out          | Out                 | Out          | **Primary**    | —      | Desktop native | Explicit-start Streamable HTTP on loopback with a session bearer token, Host/Origin checks, one approved root capability, and four bounded read-only tools. Secret-like paths, binary reads, writes, execution, and network proxying are closed. Renderer destruction, root revocation, replacement, user stop, and app quit revoke the endpoint |
...
| Deep-link protocol     | Out          | Out                 | Out          | **Primary**    | —      | Desktop native | `lingua://` protocol (see internal). Browsers cannot register custom protocols for the PWA without OS-level install |
| Local AI inference     | Research     | Out                 | Out          | Research       | Research | Hybrid (undecided) | Tracked in internal Browser WASM via transformers.js or webllm is viable for tiny models; larger models need desktop native with a local runtime (Ollama, llama.cpp). Decision pending a dedicated spike |
...
| Language intelligence  | Stub         | **Shipping**        | **Shipping (desktop)** | **Shipping (desktop)** | **Hybrid** | Hybrid | JS/TS keep Monaco's built-in service. Python and Ruby both ship renderer adapters for diagnostics + completions + hover + signature help in web + desktop (Ruby joined in implementation via `src/renderer/languageIntelligence/ruby.ts` — block-balance + delimiter diagnostics, local symbol-aware completions, hover with definition line, signature help for local method calls). Rust + Go ship the same surface via rust-analyzer / gopls over the shared desktop LSP bridge (`src/main/lsp/*`, implementation); capability gates on binary detection, web build degrades to `'missing'`. internal is now closed in full |
| Debugger (JS / TS)     | Out          | **Shipping**        | Out          | **Shipping**   | —      | Browser interpreter | Acorn + magic-string instrumentation in `src/renderer/runtime/debuggerInstrument.ts` rewrites user source to yield before each statement; the JS worker drives the pause loop. TS rides the same worker after esbuild transpile, with the TS→JS source map composed via `@jridgewell/trace-mapping` so breakpoints round-trip to original TS lines. Normal, conditional, and logpoint modes plus persistent watches ship in web and desktop. Debugger expressions use a separate Acorn-parsed, bounded data-only interpreter over detached locals; calls, mutations, prototype traversal, inherited access, and dynamic evaluation are rejected (see `DEBUGGER_ADR.md`) |
| Debugger (Python)      | Out          | Out                 | Out          | **Shipping** | — | Desktop native | `src/main/pythonDebugger.ts` drives a bounded headless `python -u -m pdb <script>` subprocess without a shell. The owner-bound `debugger:python:*` IPC/preload bridge and lazy renderer adapter reuse the gutter, Debugger panel, step shortcuts, locals, source-local call stack, watches, run output, and stop lifecycle. Main prefers project `.venv`/`venv`, uses a filtered environment, and removes the temporary source/process tree on every owner lifecycle. Normal Run remains Pyodide and web disables Debug. Python watches execute in the native process and may have side effects; conditional breakpoints/logpoints remain JS/TS-only |
| Debugger (Go)          | Out          | Out                 | Out          | **Shipping** | — | Desktop native | `src/main/goDebugger.ts` owns `dlv dap` on an ephemeral loopback port and `src/main/debugger/dapClient.ts` applies bounded DAP framing. The owner-bound typed bridge and shared lazy native renderer lifecycle expose pause breakpoints, stepping, locals, source-local call stack, watches, output, and stop. Main filters the Go environment and removes the Delve/debuggee process tree plus private temporary module on every lifecycle. Normal Run is unchanged; web disables Debug, and missing Delve or macOS Developer Tools access surfaces actionable guidance |
| Debugger (Rust)        | Out          | Out                 | Out          | **Shipping** | — | Desktop native | `src/main/ipc/rustDebugger.ts` compiles the bounded current buffer with `rustc --edition 2021 -g`, then `src/main/rustDebugger.ts` drives host `lldb-dap` over bounded stdio DAP. The owner-bound typed bridge reuses pause breakpoints, stepping, locals, source-local call stack, watches, output, and stop. Main resolves Xcode's adapter through `xcrun` on macOS or `lldb-dap` on `PATH`, filters the Rust environment, and removes the adapter/debuggee process tree plus private source and binary on every lifecycle. Normal Run is unchanged; web disables Debug, and missing compiler/adapter, compile diagnostics, or macOS permission denial receive distinct guidance |
| JS/TS runtime: Worker  | **Shipping** | **Shipping**        | **Shipping** | **Shipping**   | —      | Browser interpreter (default) | implementation ships the contract surface. Web Worker isolated context; no DOM, no Node built-ins. Default for every JS/TS tab. Lives in `src/renderer/runners/javascript.ts` + `src/renderer/runners/typescript.ts` |
| JS/TS runtime: Node    | Out          | Out                 | Research     | **Shipping** | — | Desktop native | implementation (2026-05-14). Spawns Node through the main-process `child_process.spawn` backend without shell interpolation; source travels by argv / temp file, env comes from `NODE_TOOLCHAIN_KEYS` plus user env, saved tabs resolve cwd near `node_modules`, and parent-owned Stop / timeout sends SIGTERM before SIGKILL. Web builds do not expose the desktop bridge and report a desktop-only runtime error |
| JS/TS runtime: Browser preview | **Shipping** | **Shipping** | Out | **Shipping** | — | Browser interpreter | implementation (2026-05-12). Iframe-isolated DOM context with strict sandbox (`allow-scripts` only) + srcdoc CSP (`default-src 'none'`; `unsafe-inline` script + style only). Renders inside the new "Browser preview" bottom-panel tab; runId-anchored postMessage bridge for console + error capture; parent-owned timeout clears `srcdoc`; scoped sibling `.css` / `.html` tabs of the tab named by `ExecutionContext.tabId` auto-seed the document |
| Dependency management | **Shipping (Python micropip)** | **Shipping (detection + Python micropip)** | Research | **Shipping (full hybrid)** | **Hybrid** | Detection on web + desktop; JS/TS install desktop-only via npm; Python install web-only via Pyodide micropip | implementation (2026-05-22) + implementation (2026-05-24) + implementation (2026-05-25; closes internal). implementation ships detection + the adapter registry. implementation ships the JS/TS desktop install via `installJsDependencyBatch` (`child_process.spawn('npm', ['install', …, '--no-audit', '--no-fund', '--no-progress', '--save'], { shell: false })`); refuses without an upstream `package.json`, pre-flights against the implementation resolver, SIGTERM→SIGKILL cancel, 5-min timeout, log streaming via `webContents.send('dependencies:js:install:log', …)`, closed-enum telemetry. **implementation** ships the Python web install via Pyodide `micropip` — new worker handlers `dependencies:list-loaded` + `dependencies:install` in `python-worker.ts` route through `micropip.install([...])`; service module shares the Pyodide worker with `PythonRunner` so installed packages are visible to the next Run; 90 s `Promise.race` soft timeout; new `'unsupported-wheel'` failure reason for Pyodide native-wheel rejections; loaded-packages bridge marks pre-loaded packages as `'installed'`. Cancel hidden on the Python web path because Pyodide has no mid-microtask kill primitive. Promotion rules for Ruby / Go / Rust / Python-venv / WebContainer install adapters live in the internal dependency-manager ADR |
| Git read-only layer    | Out          | Out                 | Out          | **Shipping**   | —      | Desktop native | implementation (2026-05-22). `src/main/git.ts` exposes `detectGit` / `getFileStatus` / `getFileDiff` over `execFile` (shell:false, 5s timeout, 64 KiB cap per side). Per-file pill in `<EditorTabs>` (clean/modified/untracked/unknown) + Monaco diff editor in a bottom-panel sibling tab. Web build leaves `window.lingua.git` absent and the renderer's `bridge?.git` guards short-circuit the pill + panel. implementation adds `.git/HEAD` watching for live branch refresh + external-mod reload notice; implementation adds the write surface (add/commit/branch switch) behind explicit gates |
| SQL workspace          | **Shipping** | **Shipping**        | Out          | **Shipping**   | —      | Browser WASM | DuckDB-WASM (MVP bundle) runs entirely in the renderer worker in web + desktop; no main-process path. `src/renderer/runtime/duckdbClient.ts` owns the lazy singleton, the Arrow→JSON mapping (10k-row / 256 KiB preview caps), and OPFS opt-in persistence with CHECKPOINT-on-write. Web self-hosts / R2-mirrors the ~7 MiB WASM with sha256 verification. Same execution class both platforms — desktop adds nothing here |
| Notebook workspace     | **Shipping** | **Shipping**        | Out          | **Shipping**   | **Hybrid** | Browser interpreter + hybrid (Python) + Browser WASM (SQL) | TS/JS cells run in the shared worker (`notebookSession.ts` → `runnerManager`); Python cells ride the same Pyodide path (web + desktop) as the Python runner, hence hybrid. implementation gives Python cells a **per-notebook kernel scope**: the worker runs each cell against a namespace dict keyed by the notebook tabId (`notebookSession` passes `scopeId`), so cells in one notebook share Python state while notebook A / notebook B / the editor scratchpad stay isolated; Restart kernel + tab close send `reset-scope`. See `PYTHON_NOTEBOOK_KERNEL_ADR.md`. implementation adds SQL cells that run on the shared DuckDB-WASM engine (`executeQuery`) — the result set is emitted as a JSON-array output the rich-output layer renders as a table, and because the engine is a renderer-wide singleton, SQL cells share table state with each other and the SQL workspace. `'sql'` is a notebook-only cell language (NOT a global editor `LanguagePackId`). Lazy cross-language reactivity is conservative and document-ordered: edits or settled runs mark the affected previously executed suffix stale, preserve old output, and never auto-run. Explicit refresh resets the notebook-owned JS/TS sandbox and Python scope before replaying the validated execution prefix; SQL statements replay against the shared DuckDB connection without promising rollback. Persistence keeps the notebook plus a bounded execution-order ledger so silent setup cells survive reload (`.linguanb` disk autosave remains pending). Cell list is doubly virtualized; only the focused cell mounts Monaco |
| HTTP workspace         | **Shipping** | **Shipping**        | Out          | **Shipping**   | —      | Browser APIs (web) + desktop native proxy | Ordinary HTTP, SSE, WebSocket sessions, and sequential HTTP pipelines share `HttpRequestV1`, environment interpolation, capture chaining, assertions, cancellation, and response caps. Web uses browser `fetch` / `WebSocket`, so CORS, mixed-content, forbidden headers, and private-network policy still apply. Desktop routes through typed IPC: `src/main/httpProxy.ts` checks every HTTP/SSE hop, rejects private ranges by default, strips credential-bearing headers on cross-origin redirects, and DNS-pins the cleared addresses through undici; `src/main/httpWebSocket.ts` applies the same host guard and pinning with redirects disabled. Settings exposes an explicit desktop-only private-host opt-in. Live previews and recorded responses are scrubbed before display/persistence |
...
- **Use the lightest adapter that can ship in both shells first.** Python's
  implementation adapter runs in the renderer, emits Monaco markers under the
  `lingua-language-intelligence` owner, and augments completions from local
  symbols without requiring a desktop bridge. implementation layers hover (kind +
...
- **Fengari is pinned-and-unmaintained — known, accepted.** `fengari@0.1.5`
  ...
  security patch stream for the Lua VM. Accepted under the internal trust
...
### Ruby
- **Browser WASM via `@ruby/wasm-wasi` is the shipping default.** implementation wires `src/renderer/workers/ruby-worker.ts` + `src/renderer/runners/ruby.ts`
  around a persistent web worker hosting CRuby + stdlib (Ruby 3.4 head from
  the `@ruby/3.4-wasm-wasi` package). Parent-owned deadline per internal plus
...
- **Bundled stdlib only.** `require 'json'` / `require 'date'` work because
  ...
  slice paired with the internal package-management lane.
- **Renderer-side language intelligence ships in the same slice.**
  ...
  per internal, so a Ruby tab pulls its tokenizer + provider chunks on
  demand) provides
  ...
  bridge would land in a separate slice under internal
- **Desktop subprocess shipped in implementation** `src/main/ruby-runner.ts`
  spawns the host `ruby` via `child_process.spawn` (no shell, tempfile
  source), with the internal native-env allowlist + internal user env tier,
...
|------------|------------------|-------------------------------------------|
| 2026-04-17 | internal drafters  | Initial matrix and decision records landed |
| 2026-05-11 | implementation   | Python renderer adapter added for diagnostics and symbol-aware completions |
