...
| Dependency management | Out | **Detection only (web)** | Research | **Shipping (detection)** | **Hybrid** | Hybrid (detection web + desktop, install desktop-first) | RL-025 Slice A (2026-05-22). `src/shared/dependencies/{javascriptDetector,pythonDetector,registry}.ts` detects external imports without IPC; main-side `resolveJsDependencyBatch` walks `node_modules` for JS/TS; web build short-circuits to `'needs-desktop'`. Slice B will add the JS/TS desktop install path via `child_process.spawn` (anti-feature: never silent, per-package confirm); Slice C adds Python web install via `micropip`. Promotion rules for Ruby / Go / Rust / Python-venv / WebContainer install adapters live in `docs/DEPENDENCY_MANAGER_ADR.md` |
