...
| Notebook workspace     | **Shipping** | **Shipping**        | Out          | **Shipping**   | **Hybrid** | Browser interpreter + hybrid (Python) | TS/JS cells run in the shared worker (`notebookSession.ts` → `runnerManager`); Python cells ride the same Pyodide (web) / native (desktop) path as the Python runner, hence hybrid. Persistence is localStorage today (`.linguanb` disk autosave is the pending slice). Cell list is doubly virtualized; only the focused cell mounts Monaco |
