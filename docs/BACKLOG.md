...
- [licensing] Slice 3.5 — desktop main-side `/licenses/activate` + `/licenses/status` wiring. The Slice 0 IPC bridge in `src/main/license.ts` is still local-verify-only, so desktop users effectively bypass the per-surface device cap that the web build enforces and never see the device list in Settings → License. Closing the gap means adding fetch wrappers to main, threading the device id from `userData/device-id.json` into the activate body, and exposing the `devices` + `deviceLimit` snapshot through `window.lingua.license.*` to the renderer. AC still TBD. — 2026-04-28 (jy)
