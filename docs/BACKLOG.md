...
- [ui] Promote Run Capsule export to a primary surface in the result-panel header (RL-094 Slice 1 follow-up). Today the export lives only in Settings → Account → Run Capsules (3 clicks from the result the user just looked at) and the command palette (`Export latest run as capsule`). Settings is a category mismatch — it's a "config / audit" surface, not a "single-shot action on session state" surface. Add an icon button next to Compare / Variables / hideUndefined in `<ResultPanelHeader>` that fires the same sanitize → JSON.stringify (pretty) → clipboard flow already wired in `RunCapsulesSection` + `CommandPalette`; falls back to the existing inline textarea path when clipboard rejects. Surfaces only when `latestCapsule()` is truthy so it never advertises a no-op. Telemetry tagged `trigger: 'result-panel-export'` (closed enum widened in `CAPSULE_EXPORT_TRIGGERS` + parity test); update-server mirror in the same commit. Atajo candidato: `Cmd+Shift+E` (no colisión con `KEYBOARD_SHORTCUTS`). Settings entry + palette command se mantienen como audit + power-user surfaces (mismo split que la toggle de auto-log: preference en Settings, acción per-tab en el floating action pill). 1 nueva i18n key x 2 locales (tuteo: "Exportar"). Estimación ~50 LOC + 2 tests (visible-when-capsule, click-dispatches-clipboard). — 2026-05-21 (jy)
