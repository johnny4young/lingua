...
P3/P4/P7 de rendimiento, el split final IT2-A1 de filesystem y los seis
...
`fs/promises`; los cuatro componentes renderer que aún superan 800 LOC siguen
abiertos. Evidencia
...
`output/review/a5-editor-section-split/` y el de `UtilityPipelinePanel` en
`output/review/a5-utility-pipeline-split/`.

Siguiente secuencia recomendada: los cuatro componentes restantes de A5,
comenzando por `HttpRequestEditor`. G3 ya cumplió el gate de medición y P6
...
> **Estado actual.** Se partieron seis componentes confirmados por la
...
> streaming (91). El inventario bajó de 10 a 4 componentes sobre 800 LOC y un
> test estructural fija los budgets de padres y módulos extraídos. Restan
> `HttpRequestEditor`, `SettingsModal`, `ConsolePanel` y `SqlWorkspacePanel`;
> no se marcan como hechos.

- **Objetivo.** Diffs revisables; ningún cambio de comportamiento.
- **Siguiente orden por retorno:** `HttpRequestEditor` (964), `SettingsModal`
  (864), `ConsolePanel` (839) y `SqlWorkspacePanel` (829).
