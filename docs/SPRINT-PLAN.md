...
|------|--------|:------:|-------|
| Iter 1 | [`RL-068`](./ROADMAP.md) · [`RL-072`](./ROADMAP.md) | Shipping · RL-070 code-conversion closeout landed 2026-04-24 (RL-070 + RL-071 Done) | Expand Developer Utilities to DevUtils parity — QR Code generate; JWT Decode/Verify/Sign modes covering all 12 JWS algorithms; Base64 Image Encode/Decode panel; Beautify/Minify covering JSON + JS (terser v5) + HTML + CSS + SCSS + LESS + XML; Backslash Escape/Unescape (JS / JSON / Python / SQL-MySQL); Random String Generator (Web Crypto rejection sampling); Lorem Ipsum Generator; Regex Replace mode; SVG → CSS converter; Cron Parser (cron-parser + cronstrue lazy-imports); Hash Generator closeout (MD5 via spark-md5, SHA-384/512, HMAC SHA family, file-drop); HTML → JSX converter (DOMParser-based attribute translation + style-object literal); cURL → Code converter across fetch / undici / requests / net/http targets. See §3 for the closing summary. |
