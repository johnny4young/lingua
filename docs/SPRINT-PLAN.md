...
| Iter 25 | [`RL-020`](./ROADMAP.md) | Slice 1 shipped (2026-05-13) | Scratchpad / REPL excellence — Slice 1 ships the smart auto-run completion gate for JS/TS: pure-string scanner with bracket / quote / template / block-comment balance, trailing operator + keyword checks, and an auto-pair-aware sweep that catches Monaco-inserted close brackets; runner short-circuits before the iframe / Web Worker spins up; last-successful-snapshot preserve + restore in `resultStore`; ambient `<AutoRunGateNotice>` footer with browser-preview-specific copy variant (fold E); closed-enum telemetry `runtime.auto_run_gated` (fold A) mirrored on update-server with a parity guard; vitest bench locks the heuristic under 150 µs / call (fold F). Remaining: scope items 3–9 (inline watches, auto-log, stdin, timeout presets, per-tab history, variable inspector). See §18. |
