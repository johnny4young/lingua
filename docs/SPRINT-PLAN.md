...
|------|--------|:------:|-------|
| Iter 1 | [`RL-068`](./ROADMAP.md) · [`RL-072`](./ROADMAP.md) (QR Code) + [`RL-071`](./ROADMAP.md) (JWT all 12 JWS algorithms + Base64 Image) + [`RL-070`](./ROADMAP.md) (HTML + CSS + XML Beautify/Minify) + [`RL-068`](./ROADMAP.md) (Backslash Escape/Unescape + Random String Generator) all shipped 2026-04-23 | Shipped | Expand Developer Utilities to DevUtils parity — QR Code generate; JWT Decode/Verify/Sign modes covering all 12 JWS algorithms (HS256/384/512 + RS256/384/512 + ES256/384/512 + PS256/384/512); Base64 Image Encode/Decode panel (drag-drop file → data-URI, paste data-URI → preview, MIME + size validation); HTML + CSS + XML Beautify/Minify (Prettier postcss/html/xml plugins + whitespace-only minifiers with preserve semantics per language); Backslash Escape/Unescape (JS / JSON / Python / SQL-MySQL presets); Random String Generator (configurable length + count + 4-class charset toggles + ambiguous exclusion, unbiased Web Crypto rejection sampling). See §3 for the closing summary. |
