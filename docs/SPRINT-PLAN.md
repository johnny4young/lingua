...
| Iter 30 | [`RL-044`](./ROADMAP.md) Slice 1A | Shipped (2026-05-18) | Rich output foundation + `//=> table` directive. New `src/shared/richOutput.ts` (RichOutputPayload discriminator superset of ScopeValue + 8 new kinds, `serializeRichValue`, `detectAutoTable`, `forceTablePayload`, `tryParseJsonForPayload`); additive `MagicCommentResult.payload?` + `LineResult.payload?` (zero breaking changes); `magicComments.ts` parses `//=> table` directive on JS arrow + Python `#=>` arrow (unknown directives fall through to legacy arrow safely); JS + TS runners attach typed `RichOutputTable` payload when directive is set + JSON round-trip succeeds; `useInlineResults.ts` upgrades the inline pill from JSON-blob to `Table(N×M) — cols` / `Map(N)` / `Set(N)` / ISO date / `Promise(state)`. Tests cover `src/shared/richOutput.ts`, directive parsing, JS/TS runner payload stitching, and inline overlay truncation. Slice 1B (console panel widget + popover + runner payload migration) and Slice 2 (chart/image/HTML/Python detection) carry forward as separate plans documented in `docs/PLAN.md § Slice 1A landed (2026-05-18)`. |
