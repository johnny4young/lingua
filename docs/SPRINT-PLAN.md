...
| Iter 25 | [`RL-020`](./ROADMAP.md) | Slices 1–4 shipped (2026-05-13 / 2026-05-14) | Scratchpad / REPL excellence — Slices 1–3 shipped (auto-run gate, per-tab workflow mode, `@watch` magic-comment pin). **Slice 4 shipped 2026-05-14 — per-tab execution history with one-click rerun**: `ExecutionHistoryEntry.tabId?` + `pinned?` fields, `byTabId` selector + `togglePin` action with pin-aware FIFO eviction, `tabId` threaded through `executeTabManually.record()` on success + error paths, new `<RecentRunsPill>` in the result-panel header (Pro-gated; Free tier sees a fold-E upsell variant), `Mod+Shift+H` keyboard toggle (fold B) via the new `recentRunsPopoverBridge` module, "This tab only" filter checkbox in the existing `ExecutionHistoryPopover` (fold C), parallel "Recent runs (this tab)" group in the command palette ranked above the legacy global one (fold G), 60s relative-time auto-refresh (fold F), `runtime.history_replay { language, status, surface }` closed-enum telemetry mirrored on update-server with a `HISTORY_REPLAY_SURFACES` parity test (fold A). Auto-run still does not record history; replay still does not append a second entry. Remaining: scope items 5–9 (auto-log, stdin, timeout presets, variable inspector). See §18. |
