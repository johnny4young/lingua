...
| Iter 31 | [`RL-044`](./ROADMAP.md) Slice 1B | Shipped (2026-05-19) | Console panel rich rendering + popover detail surface + folds A–H + tooltip refinement. Additive `payload?: RichOutputPayload[]` on `ConsoleOutput` + `ConsoleEntry`, JS-worker `serialize()` now emits typed payloads via `serializeRichValue`, `console.table` shim with second-arg subset (fold D), 7 new components under `src/renderer/components/Console/`, `<ConsoleEntryPopover>` with Preview + Raw JSON tabs + Tooltip refinement on the shortcut surfaces (fold B + user ask), Pro-gated Copy as JSON (fold C), payload-kind filter chips (fold A), Settings master toggle (fold E), `runtime.console_rich_rendered` + `runtime.console_table_called` telemetry (mirrored on update-server with `CONSOLE_RICH_KIND_BUCKETS` parity test, fold F), 1k-iter bench guard (fold G), consecutive-entry collapse `×N` (fold H). i18n in both locales (tuteo Spanish). See `docs/PLAN.md § Slice 1B landed (2026-05-19)` for deferred Slice 1C / Slice 2 carry-over. |
