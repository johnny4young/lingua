...
| Iter 15 | [`RL-069`](./ROADMAP.md) | Slice 2 Shipped (2026-05-09) | DevUtils-class productivity layer — Slice 1 foundation (2026-05-05) shipped Cmd/Ctrl+K + Cmd+Shift+C + Cmd+Alt+R + fuzzy search + output providers on 5 panels. Slice 2 (2026-05-09) shipped `detect()` predicate on 27 panels, `<UtilityToolbar>` primitive with self-registering ⚡ Apply button, new `utility-apply-from-input` shortcut (Mod+Shift+A), full 29-panel coverage of the productivity shortcuts, mode-flip on bidirectional panels (Base64, URL, HtmlEntity, BackslashEscape, YamlJson, JsonCsv, Base64Image), 6 i18n keys per locale, and 9 e2e assertions in `tests/e2e/utilitiesApply.spec.ts`. Slice 3 (clipboard-on-focus + history + favorites) still pending. |
