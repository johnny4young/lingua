...
| Iter 4 | [`RL-061`](./ROADMAP.md) | Partial · Slices 0+1+2+2.5+3+3.5 shipped (Slice 3.5 on 2026-04-29 wires the desktop main-side bridge into `/licenses/{activate,status,devices/remove}`, mirrors the snapshot through IPC, and unlocks the Devices section on desktop builds). Remaining slices: 4 (trial+education+recovery CTAs), 5 (release pipeline + web update banner). | License-key infrastructure. Slice 0 closed the preload/main-side gap; Slice 1 laid the worker skeleton; Slice 2 wires real D1 + Polar + Resend with surface-aware device limit; Slice 2.5 brings the web build into the same server contract; Slice 3 closes the device-list remediation loop on web; Slice 3.5 lights up desktop end-to-end. See [`LICENSING_ADR.md`](./LICENSING_ADR.md) and §6. |
