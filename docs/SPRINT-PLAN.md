...
| Iter 25 | [`RL-020`](./ROADMAP.md) | Slices 1–3 shipped (2026-05-13 / 2026-05-14) | Scratchpad / REPL excellence — Slice 1 (auto-run gate) and Slice 2 (per-tab workflow mode) shipped on 2026-05-13. **Slice 3 shipped 2026-05-14 — `@watch` magic-comment pin**: discriminated-union parser for `// @watch <expr>` / `# @watch <expr>`, kind-aware side-table threaded through JS / TS / Python runners (zero worker-protocol change), `LineResult.type === 'watch'` variant rendered with a Pin icon + `aria-live="polite"`, fold C watch-survives-error splice in `useAutoRun`, fold G `hideUndefined` exemption with `magic.watch.empty` copy, fold A `runtime.magic_comment_emitted` closed-enum telemetry mirrored on update-server with parity test, fold E "Pin watch on current line" command-palette action with declaration-aware expression inference, fold D parser bench, refreshed JS / TS / Python `defaultCode` templates demoing arrow + watch side-by-side, new `clearVisibleResults` store action preserves the Slice 1 snapshot through empty-buffer transits. Remaining: scope items 4–9 (auto-log, stdin, timeout presets, per-tab history, variable inspector). See §18. |
