...
| Iter 25 | [`RL-020`](./ROADMAP.md) | Slice 1 + Slice 2 shipped (2026-05-13) | Scratchpad / REPL excellence — Slice 1 lands the auto-run completion gate (gate notice + telemetry + bench). **Slice 2 lands the per-tab workflow mode (Run / Debug / Scratchpad)**: pure `workflowMode` shared module (`default` / `supports` / `coerce` / `cycle`); `<WorkflowModeSegment>` 3-segment toolbar control next to Run; `useAutoRun` workflow-mode short-circuit as a true no-op for Run / Debug; per-language defaults map in `settingsStore` with fold-C seed for JS / TS / Python (Go / Rust default to Scratchpad to preserve auto-run); fold-A `Mod+Shift+M` cycle shortcut; fold-B `<WorkflowModeStatusPill>` mirror in the result-panel header; fold-D auto-correction telemetry on language change (`trigger: 'language_change'`); fold-E arrow-key skip on disabled segments; fold-F one-shot onboarding notice on first move away from Scratchpad; fold-G mode-aware empty-state copy; `runtime.workflow_mode_changed` closed-enum telemetry mirrored on update-server with parity tests. Remaining: scope items 3–9 (inline watches, auto-log, stdin, timeout presets, per-tab history, variable inspector). See §18. |
