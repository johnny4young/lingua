...
| [`RL-044`](./PLAN.md#rl-044-add-inline-data-visualization-and-rich-output-rendering) | Inline data visualization + rich output | `Partial` | **Slice 1C shipped 2026-05-19 — Python (Pyodide) console payload emission + all seven folds (A–G)**. The third worker-owned runtime joins JS / TS on the rich path. New Python preamble in `src/renderer/workers/python-worker.ts` ships `__lingua_console_serialize` walker (dict / list / tuple / set / frozenset / datetime / dataclass / `BaseException` → `kind:'error'` fold F / fallback `rawText`) + `__lingua_print` override + `__lingua_displayhook` REPL capture (fold A); per-arg payload capture so `print(a, b, c)` yields 3 aligned payloads (fold C); `#=> table` magic-comment forced-table upgrade (fold D); Settings master toggle flows through via `richConsoleEnabled` execute message so the worker-side serializer is skipped when the user opted out (fold E); new `runtime.python_console_payload_emitted { kind }` closed-enum telemetry mirrored on update-server with parity test (fold B); 1k-iteration / 750 ms bench guard `tests/shared/pythonConsoleSerialize.bench.test.ts` (fold G). Renderer `python.ts:291` forwards `msg.payload` into `ConsoleOutput.payload` and fires the new telemetry; magic-comment dispatch consumes the worker payload OR recovers client-side via `tryParseJsonForPayload + forceTablePayload`. Go / Rust subprocess runners stay on the text-only path pending a separate architectural slice. **Slice 1B shipped 2026-05-19 — console panel rich rendering + popover detail surface + all eight folds (A–H)** + shortcut-surface tooltip refinement. Additive `payload?: RichOutputPayload[]` on `ConsoleOutput` + `ConsoleEntry` (no fixture migration); `js-worker.ts` `serialize()` now also emits an aligned payload array via `serializeRichValue`; `console.table(rows, columns?)` shim with Chrome DevTools second-arg subset (fold D); 7 new React components under `src/renderer/components/Console/` (`ConsoleEntryRenderer` + 5 `RichValue*` + `ConsoleEntryPopover` with Preview + Raw JSON tabs); fold A payload-kind filter chips (`Tables · Objects · Arrays · Map/Set · Text · Errors`); fold B `Mod+Shift+J` Raw JSON toggle + palette entry + Tooltip refinement on both popover tabs via the existing `<Tooltip>` primitive (`⌘⇧J — Switch to Raw JSON view`); fold C Pro-gated "Copy as JSON" via the `EXECUTION_HISTORY` entitlement; fold E Settings → Editor "Rich console output" master toggle (default ON) + `Settings.consoleRichRenderingEnabled` store field; fold F separate `runtime.console_table_called` adoption telemetry; fold G `tests/shared/consoleRich.bench.test.ts` 1k-iteration / 750 ms budget; fold H consecutive-identical-entry collapse `×N` chip. New `runtime.console_rich_rendered { kind }` closed-enum telemetry (mirrored on update-server with `CONSOLE_RICH_KIND_BUCKETS` parity test). Slice 1A (2026-05-18) shipped the foundation `RichOutputPayload` type system + `//=> table` directive that upgrades the inline pill from JSON-blob to `Table(N×M) — cols`. Remaining after Slice 1C: breaking `ConsoleOutput.args: string[]` → `args: RichOutputPayload[]` migration; native payload emission for Go/Rust subprocess runners; extracting shared formatters from `VariableInspectorPanel`; `serializeRichValue` memoisation. Remaining for Slice 2 (security review required): chart/image/HTML rendering, Python matplotlib/DataFrame detection. |
