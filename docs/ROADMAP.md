...
| ID | Title | Status | Scope one-liner |
|----|-------|:------:|-----------------|
| [`RL-026`](./PLAN.md#rl-026-add-language-intelligence-beyond-monacos-built-in-jsts-services) | Language intelligence beyond Monaco | `Partial` | **Slice 2 shipped 2026-05-11** — Python now ships hover (symbol kind + definition line + parameter list) and signature help (parameter list + active-parameter tracking, triggers on `(` and `,`) on top of the Slice 1 diagnostics + completions adapter. Remaining: Go/Rust desktop-LSP adapters. |
...
7. **Debugger + language intelligence.** `RL-027` Slice 1.5 and `RL-026` Slices 1 + 2 are shipped (Slice 2 closes Python hover + signature help). Remaining work is `RL-027` Slice 1.5b (conditional breakpoints + watch expressions behind security review) and the `RL-026` Go/Rust desktop-LSP follow-up. These unblock `RL-042` and `RL-047`.
