...
| [`RL-020`](./PLAN.md#rl-020-make-the-scratchpad-and-repl-experience-best-in-class) | Scratchpad / REPL excellence | `Partial` | **Slice 1 shipped 2026-05-13** — smart auto-run completion gate for JS/TS Scratchpad: pure-string scanner detects open brackets / quotes / templates / block comments, trailing operators (`=`, `+`, `&&`, ...) and trailing keywords (`const`, `function`, `await`, ...), plus an auto-pair-aware sweep that catches Monaco-inserted close brackets (`for (let i = )`, `[1, ]`, `() => )`); short-circuits the runner so console + Browser preview iframe stop flickering between transient SyntaxErrors; preserves the last successful run via `resultStore.lastSuccessfulSnapshot` and restores it on a gated keystroke; ambient `<AutoRunGateNotice>` footer in the result panel with browser-preview-specific copy variant (fold E); closed-enum telemetry `runtime.auto_run_gated` mirrored on update-server with parity test (fold A); vitest bench locks the heuristic under 150 µs / call (fold F). Remaining: scope items 3–9 (magic-comment watch system, auto-log expressions, stdin support, timeout presets, per-tab history with rerun, variable inspector). |
