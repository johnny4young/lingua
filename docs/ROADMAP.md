...
| [`RL-020`](./PLAN.md#rl-020-make-the-scratchpad-and-repl-experience-best-in-class) | Scratchpad / REPL excellence | `Partial` | Slice 1 + Slice 2 shipped 2026-05-13 (auto-run completion gate + per-tab workflow mode). **Slice 3 shipped 2026-05-14** — `// @watch <expr>` (and Python `# @watch <expr>`) magic-comment pin for sticky scratchpad inspection: discriminated-union parser, per-line `kindByLine` side-table threaded through JS / TS / Python runners (zero worker-protocol change), new `LineResult.type === 'watch'` variant with Pin icon + `aria-live="polite"` rendering, `hideUndefined` exemption for explicit user pins, watch-survives-error stitching (fold C splices last-good watches into error-run `lineResults`), `clearVisibleResults` action so an empty-buffer transit preserves the Slice 1 snapshot, fold A `runtime.magic_comment_emitted` telemetry `{ language, hasArrow, hasWatch }` mirrored on update-server, fold E command-palette "Pin watch on current line" action with declaration-aware expression inference (`const b = 2;` → watches `b`), fold D parser bench (10 000× / 5 KB < 400 ms), refreshed JS / TS / Python `defaultCode` templates demoing arrow + watch side-by-side so the feature is discoverable on first tab. Remaining: scope items 4–9 (auto-log expressions, stdin support, timeout presets, per-tab history with rerun, variable inspector). |
