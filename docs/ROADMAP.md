...
| [`RL-061`](./PLAN.md#rl-061-polarsh-integration) | Polar.sh integration | `Partial` | Slice 3.5 shipped 2026-04-29 (desktop main bridge now calls `/licenses/{activate,status,devices/remove}` with `surface: 'desktop'`, registers in the per-surface bucket, picks up `refreshedToken` on revalidate, and re-activates when `deviceRegistered: false` so a rehydrated exhausted token cannot bypass the cap; renderer's desktop branch mirrors the new snapshot fields so the Devices section renders under the same gate the web build uses). Remaining: trial+education+recovery CTAs (Slice 4), release pipeline + web update banner (Slice 5). See [`LICENSING_ADR.md`](./LICENSING_ADR.md). |
