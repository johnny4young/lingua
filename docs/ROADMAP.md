...
| ID | Title | Status | Scope one-liner |
|----|-------|:------:|-----------------|
| [`RL-026`](./PLAN.md#rl-026-add-language-intelligence-beyond-monacos-built-in-jsts-services) | Language intelligence beyond Monaco | `Partial` | **Slice 1 shipped 2026-05-11** — Python now has a renderer adapter for editor-time diagnostics plus symbol-aware completions in web + desktop. Remaining: hover/signature help and Go/Rust desktop-LSP adapters. |
...
7. **Debugger + language intelligence.** `RL-027` Slice 1.5 and `RL-026` Slice 1 are shipped. Remaining work is `RL-027` Slice 1.5b (conditional breakpoints + watch expressions behind security review) and the broader `RL-026` hover/signature + Go/Rust LSP follow-up. These unblock `RL-042` and `RL-047`.
