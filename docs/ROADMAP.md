...
| [`RL-080`](./PLAN.md#rl-080-release-grade-desktop-ci-and-update-validation-gates) | Release-grade desktop CI + update gates | `Partial` | Slice 1 shipped 2026-05-04 — `update-server/test/index.test.ts` now covers the desktop `/update/:platform/:version` feed and download proxy with 9 tests (no-published-release, method guard, no-update, darwin happy/missing-asset, win32 happy/missing-asset, RELEASES download fails, download method guard). The worker typecheck + test suite were also wired into `ci.yml` (they had been ship-and-forget). Remaining: packaged desktop smoke against release artifacts, SHA256SUMS re-verify on publish, `npm audit --audit-level=high` blocking in `release.yml`, RELEASE.md ↔ workflow audit. |
