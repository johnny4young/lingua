...
| ID | Title | Status | Scope one-liner |
|----|-------|:------:|-----------------|
| [`RL-019`](./PLAN.md#rl-019-add-explicit-jsts-runtime-modes-worker-scratchpad-desktop-node-and-browser-preview) | Explicit JS/TS runtime modes | `Partial` | Slice 1 shipped 2026-05-12 (contract surface). **Slice 3 shipped 2026-05-12** — `BrowserPreviewRunner` with iframe + strict sandbox (`allow-scripts` only, no `allow-same-origin`) + srcdoc CSP (`default-src 'none'`); bottom-panel tab guarded on `runtimeMode === 'browser-preview'`; runId-anchored postMessage bridge; parent-owned timeout clears `srcdoc`; scoped multi-file seed for sibling `.css` / `.html` tabs (fold A); inspect button opens preview in a new opaque-origin data URL window (fold F); ADR amendment with per-mode CSP audit (fold B). Slice 2 (desktop Node child-process backend) is the only remaining sub-slice. |
| [`RL-020`](./PLAN.md#rl-020-make-the-scratchpad-and-repl-experience-best-in-class) | Scratchpad / REPL excellence | `Partial` | Slice 1 shipped 2026-05-13 (auto-run completion gate). **Slice 2 shipped 2026-05-13** — per-tab workflow mode (Run / Debug / Scratchpad) as a 3-segment toolbar control next to the Run button; pure-string `workflowMode` module with `defaultWorkflowMode(language)`, `supportsWorkflowMode`, `coerceWorkflowMode`, `cycleWorkflowMode`; `useAutoRun` short-circuits on Run / Debug as a true no-op (no `clear()`, no `lastCodeRef` advance, no `isAutoRunning` flip); per-language defaults map in `settingsStore` with fold-C seed for JS / TS / Python (Go / Rust default to Scratchpad to preserve auto-run); fold-A `Mod+Shift+M` cycle shortcut that skips unsupported segments; fold-B `<WorkflowModeStatusPill>` mirror in the result-panel header; fold-D auto-correction telemetry when a tab's language change forces a workflow flip; fold-E arrow-key navigation skips disabled segments; fold-F one-shot onboarding status notice the first time a user leaves Scratchpad; fold-G mode-aware empty-state ("Press Cmd+R to run"); closed-enum `runtime.workflow_mode_changed` telemetry with `{ language, from, to, trigger }` mirrored on update-server with parity tests. Remaining: scope items 3–9 (magic-comment watch system, auto-log expressions, stdin support, timeout presets, per-tab history with rerun, variable inspector). |
