...
| [`RL-061`](./PLAN.md#rl-061-polarsh-integration) | Polar.sh integration | `Partial` | Slice 3 shipped 2026-04-28 (web Settings → License now lists per-surface device buckets with `N of M` counters, current-device chip, per-row Remove, and an exhausted-devices remediation modal that calls `revalidate()` on open and a Retry-activation primary action; rename intentionally deferred — no `/licenses/devices/rename` endpoint yet). Remaining: trial+education+recovery CTAs (Slice 4), release pipeline + web update banner (Slice 5), and the desktop main-side `/licenses/*` activation wiring (Slice 3.5). See [`LICENSING_ADR.md`](./LICENSING_ADR.md). |
