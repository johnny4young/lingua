...
| [`RL-044`](./PLAN.md#rl-044-add-inline-data-visualization-and-rich-output-rendering) | Inline data visualization + rich output | `Partial` | Slice 1A/1B/1C shipped the `RichOutputPayload` foundation, console rich rendering, and Python payload emission. Next: payload-aware renderer migration plus chart, image, JSON tree, and sandboxed HTML payloads; keep text fallback and Settings kill switch. |
