 ...
 *
 * Locks the per-tool code-splitting win: opening the modal on the default
...
test.describe('Lazy Developer Utilities panels (RL-125)', () => {
  test('default tool skips heavy panel chunks; selecting QR loads its chunk on demand', async ({
    ...
    // Developer Utilities is Pro-gated; prime a license and wait for the badge
    // to revalidate before opening the modal.
    await seedSession(page, { language: 'en', primeProLicense: true });
    await gotoApp(page);
    await expectTier(page, 'PRO');
    await openDeveloperUtilities(page);

    // The modal opens on the JSON tool; its panel chunk loads, but the QR and
