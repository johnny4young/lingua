 ...
 *   - `ICON_GLYPH` + the box map in `src/renderer/components/ui/chrome.tsx`
 *
...
describe('icon-button density scale', () => {
  ...
  it('keeps the component box map in step with the CSS tokens', () => {
    const glyphs = CHROME.match(/ICON_GLYPH\s*=\s*\{([^}]*)\}/u);
    expect(glyphs, 'chrome.tsx must export ICON_GLYPH').not.toBeNull();
    expect(glyphs![1]).toContain(`sm: ${cssVar('icon-glyph-sm')}`);
    expect(glyphs![1]).toContain(`md: ${cssVar('icon-glyph-md')}`);

    const boxes = CHROME.match(/ICON_BUTTON_BOX[^=]*=\s*\{([^}]*)\}/u);
    expect(boxes, 'chrome.tsx must declare ICON_BUTTON_BOX').not.toBeNull();
  ...
  it('keeps call sites off ad-hoc size overrides', () => {
    ...
    const walk = (dir: string) => {
      for (const entry of readdirSync(dir)) {
        ...
        for (const block of source.match(/<IconButton[\s\S]{0,600}?>/gu) ?? []) {
