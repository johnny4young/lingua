...
describe('buildCommandPaletteModel', () => {
  ...
  it('exposes the Compare two capsules action only when the opener is wired in (RL-094 Slice 4 fold C)', () => {
    ...
    const withoutHandler = buildCommandPaletteModel(baseArgs);
    expect(
      withoutHandler.find((command) => command.id === 'action-compare-capsules')
    ).toBeUndefined();
    ...
    const command = withHandler.find(
      (entry) => entry.id === 'action-compare-capsules'
    );
    expect(command).toBeDefined();
    expect(command?.keywords).toEqual(
      expect.arrayContaining(['capsule', 'compare', 'diff'])
    );
    // Compare opens the browser (selection is inline there). `onClose()`
    // runs FIRST so the capsule-list overlay survives the shared single
    // App overlay slot — mirror of `action-browse-capsules` ordering.
    command?.action();
    expect(calls).toEqual(['close', 'browse']);
  });
