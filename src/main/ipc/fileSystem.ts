...
function coerceBundleBytes(value: unknown): Uint8Array | null {
...
export function registerFileSystemHandlers(): void {
  ...
  /**
   * RL-024 Slice 3 — pack every visible file under a capability root
   * into a `.zip` bundle and write it to a user-chosen path. `shouldHide`
   * skips `node_modules` / `.git` / `dist` / `build` etc. (fold G), so
   * the bundle stays small and a re-import never clobbers a fresh
   * install. Reads bytes (not utf-8) so binary assets round-trip. The
   * file-count + per-entry-size caps live in the shared module; we
