...
export function EditorTabs() {
  ...
  return (
    <div
      ...
      className="surface-header flex h-12 items-center gap-1 overflow-x-auto px-2 py-1.5"
    >
      {tabs.map((tab) => {
        ...
        return (
          <div
            key={tab.id}
            className={`group flex h-full items-center gap-1 rounded-2xl border pr-1.5 text-xs transition-all ${
              isActive
          ...
          >
            <button
              ...
              className="flex h-full items-center gap-2 rounded-2xl px-3.5"
            >
            ...
            {tab.isDirty && (
              <span
                aria-label={`${tab.name} has unsaved changes`}
