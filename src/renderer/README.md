...
| [`components/Debugger/`](components/Debugger)             | `DebuggerDrawer.tsx`, `DebuggerBreakpointList.tsx`, `DebuggerWatchList.tsx` | Shared JS/TS/Python pause controls and paused-frame display; advanced breakpoint modes stay JS/TS-only and Python watches disclose native side effects |
...
| [debuggerStore.ts](stores/debuggerStore.ts) | runtime-agnostic persisted breakpoint modes/watch expressions plus session-only JS/TS/Python paused frames and results; `runtime/debuggerControlBridge.ts` routes controls while `runtime/pythonDebuggerBridge.ts` lazy-loads the desktop-native adapter |
