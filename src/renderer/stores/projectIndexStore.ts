...
export const useProjectIndexStore = create<ProjectIndexState>((set, get) => ({
  ...
  refresh: async (rootPath) => {
    ...
    if (!listAllFiles) {
      ...
      set({ rootPath, status: 'idle', entries: [], error: null });
      return;
    }
    ...
    const current = get();
    if (current.status === 'loading' && current.rootPath === rootPath) {
      return;
    }

    set({ rootPath, status: 'loading', error: null });

    try {
      const raw = await listAllFiles(rootPath);
