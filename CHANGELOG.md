...
### Changed
- **Pro Lifetime is now a perpetual entitlement**: the **$59 one-time purchase unlocks Pro forever with 12 months of included updates**, replacing the previous "every future update included" promise with a sustainable model. Your Pro features never expire — a build published after your included-updates window shows a non-blocking optional-renewal notice, never a lockout. The offline Ed25519 verifier keeps a Pro Lifetime token `active` regardless of its update-window date (revocation stays authoritative through the license server when the app can sync), and the product-id family check was tightened so a look-alike token signed with the same key is rejected. Public pricing, licensing, and purchase/recovery email copy are updated to match.
