...
- **Native runners share one spawn engine**: the Node, Ruby and Rust runners now route their child-process execution through a single `spawnNativeRun` helper (`src/main/runners/spawnNativeRun.ts`) that owns the no-shell spawn, POSIX process-group leadership, parent-timeout SIGTERM→SIGKILL escalation, whole-tree kill on Stop, output byte caps + truncation, and the opt-in stdin write with its EPIPE guard. Each runner keeps its own toolchain detection, argument/temp-file construction, result-shape mapping and verbatim user-facing messages, so behavior is unchanged; per-runner differences (kill-escalation window, stdin handling) are preserved as parameters. The Go compiler stays on its own `execFile` timeout path, which does not fit the streaming-spawn shape. (Internal refactor; no user-facing behavior change.)
- **HTTP workspace — desktop SSRF-guarded proxy engine (groundwork)**: the desktop build gains a main-process HTTP engine (`src/main/httpProxy.ts`) that can bypass the browser CORS constraint while resolving every hop's host and refusing loopback / link-local / RFC 1918 / CGNAT / unique-local / multicast targets (an opt-in toggle relaxes this for private hosts), restricting schemes to http/https, and re-guarding each redirect up to a hop cap. It mirrors the existing response envelope byte-for-byte. The renderer transport seam + desktop Settings toggle that switch the workspace onto this engine are a following slice; no user-facing behavior change yet.
