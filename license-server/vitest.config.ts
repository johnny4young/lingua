 ...
 *
 * implementation doesn't touch the D1 binding, so we run tests against the Hono
 * `app.request(...)` API directly (vanilla vitest). The Hono app is just
 * a function — `await app.request(url, init, env)` returns a Response.
 *
 * implementation will need real D1 + KV emulation to exercise the trial table
 * and rate-limiter. At that point we promote to
 * `@cloudflare/vitest-pool-workers`, which spins miniflare under each
 * test. implementation deliberately stays on plain vitest to keep the bundle
 * + setup minimal.
 *
 * implementation — `loadHtmlAndCssAsText` mirrors wrangler.toml's `[[rules]]`
