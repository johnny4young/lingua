...
describe('POST /licenses/activate', () => {
  it('returns 501 not-implemented for a well-shaped body', async () => {
  ...
  it('rejects a malformed JSON body', async () => {
    const response = await app.request('http://localhost/licenses/activate', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{ "token":',
    });
...
describe('GET /licenses/status', () => {
  it('returns 501 not-implemented when the token comes via Authorization: Bearer and deviceId via query', async () => {
    const response = await app.request('http://localhost/licenses/status?deviceId=device-uuid', {
  ...
  it('rejects a request without an Authorization header — never accepts the token via URL query because logs would leak it', async () => {
    const response = await app.request(
      'http://localhost/licenses/status?token=payload.signature&deviceId=device-uuid',
    );
    expect(response.status).toBe(400);
    const body = (await response.json()) as { issues: string[] };
    expect(body.issues).toEqual(
  ...
  it('rejects a missing deviceId query param even when the Authorization header is present', async () => {
    const response = await app.request('http://localhost/licenses/status', {
      headers: { authorization: 'Bearer payload.signature' },
    });
    expect(response.status).toBe(400);
    const body = (await response.json()) as { issues: string[] };
