...
describe('POST /licenses/activate', () => {
  ...
  it('accepts an outstanding token through a verification-only overlap key', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      support_window_ends_at: issuedAt + 365 * 24 * 60 * 60,
      status: 'active',
      merchant_order_id: 'order_rotation_overlap',
      merchant_subscription_id: null,
      created_at: issuedAt,
      updated_at: issuedAt,
    });
...
describe('GET /licenses/status', () => {
  ...
  it('returns refreshedToken when a renewal replaced the persisted token for the same license row', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      merchant_subscription_id: 'sub_refresh',
      created_at: issuedAt,
  ...
  it.each([
    ...
    env.__db.licenses.set(licenseId, {
      ...
      status: 'active',
      merchant_order_id: null,
      merchant_subscription_id: 'sub_claim_mismatch',
      created_at: issuedAt,
      updated_at: issuedAt,
    });
  ...
  it('refreshes a retiring-key lifetime token to the current signing key', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      support_window_ends_at: issuedAt + 365 * 24 * 60 * 60,
      status: 'active',
      merchant_order_id: 'order_key_rotation_refresh',
      merchant_subscription_id: null,
      created_at: issuedAt,
  ...
  it('lazily re-signs the canonical row when a retiring-key token first checks status', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      support_window_ends_at: issuedAt + 365 * 24 * 60 * 60,
      status: 'active',
      merchant_order_id: 'order_key_rotation_lazy_refresh',
      merchant_subscription_id: null,
      created_at: issuedAt,
      updated_at: issuedAt,
    });
  ...
  it('keeps the first canonical token when concurrent rotations race', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      support_window_ends_at: issuedAt + 365 * 24 * 60 * 60,
      status: 'active',
      merchant_order_id: 'order_key_rotation_cas',
      merchant_subscription_id: null,
      created_at: issuedAt,
      updated_at: issuedAt,
    });
  ...
  it('withholds refreshedToken from a refunded license even when the row rotated (lifetime revocation lever)', async () => {
    ...
    env.__db.licenses.set(licenseId, {
      ...
      support_window_ends_at: issuedAt + 365 * 24 * 60 * 60,
      status: 'refunded',
      merchant_order_id: 'order_refunded',
      merchant_subscription_id: null,
      created_at: issuedAt,
      updated_at: issuedAt + 100,
    });
  ...
  it('rejects a historical token after its refresh grace window has elapsed', async () => {
    ...
    try {
      ...
      env.__db.licenses.set(licenseId, {
        ...
        status: 'active',
        merchant_order_id: null,
        merchant_subscription_id: 'sub_stale',
        created_at: issuedAt,
  ...
  it('reports expired once a canceled subscription is outside the grace window', async () => {
    ...
    try {
      ...
      env.__db.licenses.set(licenseId, {
        ...
        merchant_order_id: null,
        merchant_subscription_id: 'sub_canceled',
        created_at: issuedAt,
